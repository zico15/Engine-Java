...
public class TileMaps extends GameObject {
    ...
    private void drawTile(Tile tile, Graphics2D graphics2D){
        /*if ((tile.x_sub + grid) <= sprite.getWidth() && (tile.y_sub + grid) <= sprite.getHeight())
            graphics2D.draw(sprite.getImage(), tile.x_sub, tile.y_sub, grid, grid, tile.x, tile.y, grid, grid);*/
    }
    ...
    public void setSprite(File file) {
        sprite = new Sprite(file.getPath());
    }
